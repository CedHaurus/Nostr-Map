...
$name    = htmlspecialchars($p['cached_name'] ?: '@' . $p['slug'], ENT_QUOTES);
$bio     = $p['cached_bio']
    ? htmlspecialchars(mb_substr($p['cached_bio'], 0, 200), ENT_QUOTES)
    : 'Profil Nostr francophone sur Nostr Map.';
...
$ogImageParams = http_build_query([
    'title'  => $p['cached_name'] ?: ('@' . $p['slug']),
    'sub'    => $p['cached_bio'] ? mb_substr($p['cached_bio'], 0, 80) : 'Profil Nostr francophone',
    'avatar' => $p['cached_avatar'] ?: '',
    'slug'   => $p['slug'],
]);
$ogImage = $appUrl . '/og-image.php?' . $ogImageParams;

$imageAlt  = htmlspecialchars('Profil Nostr de ' . ($p['cached_name'] ?: $p['slug']) . ' sur Nostr Map', ENT_QUOTES);
// JSON_HEX_TAG encode < et > pour éviter que du contenu utilisateur casse le bloc <script>
$bioJson   = json_encode($p['cached_bio'] ? mb_substr($p['cached_bio'], 0, 200) : 'Profil Nostr francophone sur Nostr Map.', JSON_HEX_TAG | JSON_UNESCAPED_UNICODE);
$nameJson  = json_encode($p['cached_name'] ?: ('@' . $p['slug']), JSON_HEX_TAG | JSON_UNESCAPED_UNICODE);
$urlJson   = json_encode($url, JSON_HEX_TAG);

$ldJson = json_encode([
    '@context'   => 'https://schema.org',
    '@type'      => 'ProfilePage',
    'url'        => $url,
    'name'       => $p['cached_name'] ?: ('@' . $p['slug']),
    'description'=> $p['cached_bio'] ? mb_substr($p['cached_bio'], 0, 200) : 'Profil Nostr francophone sur Nostr Map.',
    'mainEntity' => [
        '@type'      => 'Person',
        'name'       => $p['cached_name'] ?: ('@' . $p['slug']),
        'description'=> $p['cached_bio'] ? mb_substr($p['cached_bio'], 0, 200) : 'Profil Nostr francophone sur Nostr Map.',
