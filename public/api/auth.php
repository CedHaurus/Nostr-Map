...
$db   = getDB();
$stmt = $db->prepare('SELECT npub, slug, status, last_login FROM profiles WHERE npub = ?');
$stmt->execute([$npub]);
$profile = $stmt->fetch();
...
if (!$profile) {
    ...
    $slug = $profile['slug'];
    $isFirstClaim = empty($profile['last_login']);

    if ($isFirstClaim) {
        // Fiche créée avant la première connexion de l'utilisateur
        // (admin/communauté) : le propriétaire doit pouvoir choisir son nom
        // immédiatement, même si la fiche avait été éditée auparavant.
        $db->prepare(
            'UPDATE profiles
             SET last_login = NOW(), display_name_updated_at = NULL
             WHERE npub = ?'
        )->execute([$npub]);
        logUserActivity($npub, 'claim_profile', 'profile', $npub);
    } else {
